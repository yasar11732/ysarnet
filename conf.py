...
cred = open("ftp_credentials.txt").readlines()

DEPLOY_COMMANDS = ["ftpsync.py ftp://{uname}:{psswd}@{ftp}/{remotedir} {localdir} --upload".format(
        uname=cred[1].strip(),
        psswd=cred[2].strip(),
        ftp=cred[0].strip(),
        remotedir='httpdocs',
        localdir=join(dirname(__file__),"output")
    )]
...
# (translatable)
# SEARCH_FORM = '<div style="position:fixed; right:0px; top:150px; width:150px"><div class="gcse-search"></div></div>'
#
